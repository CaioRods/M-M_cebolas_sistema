  ...
  return '<div class="tcard card-3d s-'+stKey(t,s)+'" id="card_patio_'+t.id+'" onclick="togglePatioCard(this, event)" oncontextmenu="showCtx(event,\''+t.id+'\')">'+
    ...
    '<div class="tcard-main">'+
      ...
      '<div class="pbar-container">'+
        ...
        '</div>'+
        '<div class="pbar-track"><div class="pbar-fill '+(s !== 'patio' ? 'p-ok' : 'p-warn')+'" style="width:'+pct+'%"></div></div>'+
      '</div>'+
...
/* ---------------- BACKUP / IMPORT ---------------- */
function backupJSON(){downloadBytes(strBytes(JSON.stringify({version:1,exportedAt:new Date().toISOString(),trucks,cad},null,2)),'backup-safra-tomate-'+todayISO()+'.json','application/json');toast('Backup gerado.')}
function importJSON(e){const file=e.target.files[0];if(!file)return;const r=new FileReader();
  r.onload=()=>{try{const d=JSON.parse(r.result);if(!d.trucks)throw 0;if(!confirm('Importar '+d.trucks.length+' registro(s)? Isto substituirá os dados atuais.'))return;
    trucks=d.trucks;cad=Object.assign({produtores:[],motoristas:[],placas:[],variedades:[]},d.cad||{});save();saveCad();refreshAll();toast('Dados importados.');}catch(err){toast('Arquivo inválido.','err')}};
...
function refreshAll(){fillDatalists();updateNavCounts();renderView(currentView)}
...
function init(){
  ...
  seedIfEmpty();renderChegada();updateNavCounts();
